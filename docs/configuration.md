...
Cursor ship and scout launches prepend a protected PATH directory so `gh` resolves to the enrolled native read helper when that directory is installed; [`docs/gh-read-helper.md`](gh-read-helper.md) owns the helper, and [`bin/fm-gh-read.sh`](../bin/fm-gh-read.sh) owns install and verification.
Pi, Herdr, Firstmate, interactive shells, Cursor secondmates, and generic `/usr/local/bin/gh` are unchanged.
