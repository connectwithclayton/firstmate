...
Cursor ship and scout workers reach GitHub through a dedicated native reader, `fm-gh-read`, instead of generic `gh` or `gh-axi`'s interpreter.
...
Only a Cursor ship or scout launch prepends the protected PATH directory whose `gh` entry links to the enrolled helper.
